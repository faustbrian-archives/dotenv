...
const envFile = readFileSync(resolve(__dirname, "./fixtures/example.env")).toString();
...
test("respects the whitelist", () => {
	expect(subject.whitelist(["LOG_CHANNEL"]).parse(envFile).all()).toEqual({ LOG_CHANNEL: "stack" });
});

test("respects the blacklist", () => {
	expect(subject.blacklist(["LOG_CHANNEL"]).parse(envFile).all()).not.toContainKey("LOG_CHANNEL");
});
...
test("can parse and handle empty values", () => {
	expect(() => subject.parse(envFile).required("IS_EMPTY").notEmpty().validate()).toThrowError(
...
test("can parse and allow or disallow certain values", () => {
	...
	expect(() => env.required("ALLOWED").allowedValues(["World"]).validate()).toThrowError("must be");

	env.required("ALLOWED").disallowedValues(["World"]).validate();
	expect(() => env.required("ALLOWED").disallowedValues(["Hello"]).validate()).toThrowError("invalid value");
});

test("can load a dotenv file", () => {
	expect(subject.load(resolve(__dirname, "./fixtures/path.env")).all()).toEqual({ HELLO: "WORLD" });
});

test("can safe load a dotenv file", () => {
	expect(subject.safeLoad(resolve(__dirname, "./fixtures/fake.env")).all()).toEqual({});
});
